...
if [[ ! -d $BACKUP_DIR/$DIRNAME$NO ]]; then
	...
	(( $? )) && { echo "Error creating directory $BACKUP_DIR/$DIRNAME$NO"; exit 1; }
fi
...
(( failure=failure || $? )) && { writeToConsole "DB dump of $DB_BACKUPSOURCE_DBNAME failed"; exit 1; }
...
if (( $CLONE )); then
	...
	if [[ ! -d $REMOTE_MP/$FS_RESTORE ]]; then
		...
		(( $? )) && { writeToConsole "Error creating directory $REMOTE_MP/$FS_RESTORE"; exit 1; }
	fi
