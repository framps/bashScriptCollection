...
if [[ ! -d $BACKUP_DIR/$DIRNAME$NO ]]; then
    ...
    (($?)) && {
        echo "Error creating directory $BACKUP_DIR/$DIRNAME$NO"
        exit 1
    }
fi
...
((failure = failure || $?)) && {
    writeToConsole "DB dump of $DB_BACKUPSOURCE_DBNAME failed"
    exit 1
}
...
((failure = failure || $?)) && {
    writeToConsole "Restore of DB dump to $DB_RESTORESOURCE_DBNAME failed"
    exit 1
}
RUN_TIME="$(timerEnd $RUN_TIME)"
...
if (($CLONE)); then
    ...
    if ! isMounted $REMOTE_MP; then
        mount $REMOTE_MP
        (($?)) && {
            writeToConsole "Error mounting $REMOTE_MP"
            exit 1
        }
    else
    ...
    ((failure = failure || $?)) && {
        writeToConsole "rsync down failed"
        exit 1
    }
    RUN_TIME="$(timerEnd $RUN_TIME)"
    ...
    if [[ ! -d $REMOTE_MP/$FS_RESTORE ]]; then
        ...
        (($?)) && {
            writeToConsole "Error creating directory $REMOTE_MP/$FS_RESTORE"
            exit 1
        }
    fi
    ...
    ((failure = failure || $?)) && {
        writeToConsole "rsync up failed"
        exit 1
    }
    RUN_TIME="$(timerEnd $RUN_TIME)"
